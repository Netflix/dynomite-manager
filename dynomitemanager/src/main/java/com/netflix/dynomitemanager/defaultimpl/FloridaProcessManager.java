...
public class FloridaProcessManager implements IFloridaProcess {
		...
		@Inject public FloridaProcessManager(IConfiguration config, Sleeper sleeper, InstanceState instanceState,
				...
				if (config.getClusterType()
						...
						if (!dynomiteRedisCheck()) {
								try {
										...
										logger.error("Dynomite cannot be restarted --> Requires manual restart" + e
												.getMessage());
								}
						}
				}
