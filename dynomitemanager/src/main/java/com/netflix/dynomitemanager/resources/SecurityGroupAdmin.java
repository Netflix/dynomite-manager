 ...
 */
@Path("/v1/secgroup") @Produces(MediaType.TEXT_PLAIN) public class SecurityGroupAdmin {
		...
		@Inject public SecurityGroupAdmin(IMembership membership) {
				this.membership = membership;
		}

		@POST public Response addACL(@QueryParam("ip") String ipAddr, @QueryParam("fromPort") int fromPort,
		...
		@DELETE public Response removeACL(@QueryParam("ip") String ipAddr, @QueryParam("fromPort") int fromPort,
				@QueryParam("toPort") int toPort) {
				if (!ipAddr.endsWith(CIDR_TAG))
						ipAddr += CIDR_TAG;
				try {
