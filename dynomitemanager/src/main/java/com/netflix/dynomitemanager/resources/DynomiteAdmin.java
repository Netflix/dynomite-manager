...
@Path("/v1/admin") @Produces(MediaType.APPLICATION_JSON) public class DynomiteAdmin {
		...
		@Inject public DynomiteAdmin(IFloridaProcess dynoProcess, InstanceIdentity ii, InstanceState instanceState,
		...
		@GET @Path("/start") public Response dynoStart() throws IOException, InterruptedException, JSONException {
		...
		@GET @Path("/stop") public Response dynoStop() throws IOException, InterruptedException, JSONException {
				instanceState.setIsProcessMonitoringSuspended(true);
				dynoProcess.stop();
				return Response.ok(REST_SUCCESS, MediaType.APPLICATION_JSON).build();
		}

		@GET @Path("/startstorageprocess") public Response storageProcessStart()
				throws IOException, InterruptedException, JSONException {
				storageProcessMgr.start();
				return Response.ok(REST_SUCCESS, MediaType.APPLICATION_JSON).build();
		}

		@GET @Path("/stopstorageprocess") public Response storageProcessStop()
				throws IOException, InterruptedException, JSONException {
				storageProcessMgr.stop();
				return Response.ok(REST_SUCCESS, MediaType.APPLICATION_JSON).build();
		}

		@GET @Path("/info") public Response dynoInfo() throws IOException, InterruptedException, JSONException {
		...
		@GET @Path("/ring/{id}") public Response dynoRing(@PathParam("id") String keyspace)
				throws IOException, InterruptedException, JSONException {
		...
		@GET @Path("/repair") public Response dynoRepair(@QueryParam("sequential") boolean isSequential,
		...
		@GET @Path("/version") public Response version() throws IOException, ExecutionException, InterruptedException {
				logger.info("REST: version");
				return Response.ok(new JSONArray().put("1.0.0"), MediaType.APPLICATION_JSON).build();
		}

		@GET @Path("/move") public Response moveToken(@QueryParam(REST_HEADER_TOKEN) String newToken)
				throws IOException, ExecutionException, InterruptedException, ConfigurationException {
				return Response.ok(REST_SUCCESS, MediaType.APPLICATION_JSON).build();
		}

		@GET @Path("/drain") public Response dynoDrain() throws IOException, ExecutionException, InterruptedException {
				return Response.ok(REST_SUCCESS, MediaType.APPLICATION_JSON).build();
		}

		@GET @Path("/get_seeds") public Response getSeeds() {
				try {
		...
		@GET @Path("/cluster_describe") public Response getClusterDescribe() {
				try {
		...
		@GET @Path("/backup") public Response doBackup() {
				try {
		...
		@GET @Path("/restore") public Response doRestore() {
				try {
		...
		@GET @Path("/takesnapshot") public Response takeSnapshot() {
				try {
		...
		@GET @Path("/Status") public Response floridaStatus() {
				try {
						...
						if (!this.instanceState.firstBootstrap()) {
								if (this.instanceState.isBootstrapping()) {
										...
										case EXPIRED_BOOTSTRAPTIME_FAIL:
												warmupJson.put("status",
														"failed: too fast to warmup - expired bootstrap time");
												break;
    		...
    		/* backup status */
						...
						if (!this.instanceState.firstBackup()) {
								...
								} else if (!this.instanceState.isBackingup() && !this.instanceState
										.isBackupSuccessful()) {
    		...
    		/* restore status */
						...
						if (!this.instanceState.firstRestore()) {
								...
								} else if (!this.instanceState.isRestoring() && !this.instanceState
										.isRestoreSuccessful()) {
