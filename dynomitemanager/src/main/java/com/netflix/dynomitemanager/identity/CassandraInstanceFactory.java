...
@Singleton public class CassandraInstanceFactory implements IAppsInstanceFactory {
		...
		@Inject public CassandraInstanceFactory(IConfiguration config, InstanceDataDAOCassandra dao) {
				this.config = config;
				this.dao = dao;
		}
		...
		public void sort(List<AppsInstance> return_) {
				Comparator<? super AppsInstance> comparator = new Comparator<AppsInstance>() {
						...
						@Override public int compare(AppsInstance o1, AppsInstance o2) {
								Integer c1 = o1.getId();
								Integer c2 = o2.getId();
		...
		@Override public void attachVolumes(AppsInstance instance, String mountPath, String device) {
				throw new UnsupportedOperationException("Volumes not supported");
		}

		@Override public AppsInstance getInstance(String appName, String dc, int id) {
