...
@Singleton public class InstanceDataDAOCassandra {
		...
		@Inject public InstanceDataDAOCassandra(IConfiguration config, HostSupplier hostSupplier)
		...
		private AstyanaxContext<Keyspace> initWithThriftDriverWithEurekaHostsSupplier() {
				...
				return new AstyanaxContext.Builder().forCluster(BOOT_CLUSTER).forKeyspace(KS_NAME)
						...
						.withConnectionPoolConfiguration(
								new ConnectionPoolConfigurationImpl("MyConnectionPool").setMaxConnsPerHost(3)
		...
		private AstyanaxContext<Keyspace> initWithThriftDriverWithExternalHostsSupplier() {
				...
				return new AstyanaxContext.Builder().forCluster(BOOT_CLUSTER).forKeyspace(KS_NAME)
						...
						.withConnectionPoolConfiguration(
								new ConnectionPoolConfigurationImpl("MyConnectionPool").setMaxConnsPerHost(3)
										.setPort(thriftPortForAstyanax)).withHostSupplier(getSupplier())
						.withConnectionPoolMonitor(new CountingConnectionPoolMonitor())
						.buildKeyspace(ThriftFamilyFactory.getInstance());
		...
		private Supplier<List<Host>> getSupplier() {
				...
				return new Supplier<List<Host>>() {
						...
						@Override public List<Host> get() {
								...
								List<String> cassHostnames = new ArrayList<String>(Arrays.asList(
										StringUtils.split(config.getCommaSeparatedCassandraHostNames(), ",")));
