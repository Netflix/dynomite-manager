...
public class InstanceDataDAOCassandra
{
	 ...
	 */
	public ColumnFamily<String, String> CF_TOKENS = new ColumnFamily<String, String>(CF_NAME_TOKENS, StringSerializer.get(), StringSerializer.get());
	...
	public void deleteInstanceEntry(AppsInstance instance) throws Exception
	{
		...
		// Delete the row
		String key = findKey(instance.getApp(), String.valueOf(instance.getId()), instance.getDatacenter(), instance.getRack());
		if (key == null)
	...
	public Set<AppsInstance> getAllInstances(String app)
	{
		...
		try {
			...
			final String selectClause = String.format("SELECT * FROM %s USING CONSISTENCY LOCAL_QUORUM WHERE %s = '%s' ", CF_NAME_TOKENS, CN_APPID, app);
