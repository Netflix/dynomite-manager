...
public class InstanceIdentity
{
    ...
    private final ListMultimap<String, AppsInstance> locMap = Multimaps.newListMultimap(new HashMap<String, Collection<AppsInstance>>(), new Supplier<List<AppsInstance>>()
    {
    ...
    public class GetDeadToken extends RetryableCallable<AppsInstance>
    {
        ...
        public AppsInstance retriableCall() throws Exception
        {
            ...
            for (AppsInstance dead : allIds)
            {
                ...
                return factory.create(config.getAppName(), dead.getId(), config.getInstanceName(), config.getHostname(), config.getHostIP(), 
                                      config.getZone(), dead.getVolumes(), payLoad, config.getRack());
            }
            return null;
        }
    ...
    public class GetPregeneratedToken extends RetryableCallable<AppsInstance>
    {
        ...
        public AppsInstance retriableCall() throws Exception
        {
            ...
            for (AppsInstance dead : allIds)
            {
                ...
                if (!dead.getRack().equals(config.getRack()) || asgInstances.contains(dead.getInstanceId()) || !isInstanceDummy(dead))
                    continue;
    ...
    public class GetNewToken extends RetryableCallable<AppsInstance>
    {
        ...
        public AppsInstance retriableCall() throws Exception
        {
            ...
            logger.info(String.format("Trying to createToken with slot %d with rac count %d with rac membership size %d with dc %s",
                                       my_slot, membership.getRacCount(), membership.getRacMembershipSize(), config.getRegion()));
            ...
            String payload = tokenManager.createToken(my_slot, membership.getRacMembershipSize(), config.getRack());
