...
@Singleton public class InstanceIdentity {
		...
		private final Predicate<AppsInstance> differentHostPredicate = new Predicate<AppsInstance>() {
				@Override public boolean apply(AppsInstance instance) {
		...
		@Inject public InstanceIdentity(IAppsInstanceFactory factory, IMembership membership, IConfiguration config,
		...
		public void init() throws Exception {
				...
				myInstance = new RetryableCallable<AppsInstance>() {
						@Override public AppsInstance retriableCall() throws Exception {
								...
								for (AppsInstance ins : factory.getAllIds(config.getAppName())) {
										logger.debug(
												String.format("[Alive] Iterating though the hosts: %s My id = [%s]",
				...
				@Override public AppsInstance retriableCall() throws Exception {
						...
						for (AppsInstance dead : allIds) {
								...
								return factory.create(config.getAppName(), dead.getId(), config.getInstanceName(),
										config.getHostname(), config.getHostIP(), config.getZone(), dead.getVolumes(),
										payLoad, config.getRack());
						}
						return null;
				}
		...
		public class GetNewToken extends RetryableCallable<AppsInstance> {
				...
				public AppsInstance retriableCall() throws Exception {
						...
						logger.info(String.format(
								...
								my_slot, membership.getRacCount(), membership.getRacMembershipSize(),
								config.getRegion()));
