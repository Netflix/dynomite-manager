...
public class JedisUtils {
	...
	private static final DynamicLongProperty minRetryMs =
			DynamicPropertyFactory.getInstance().getLongProperty("dynomitemanager.storage.isAlive.retry.min.ms", 3000L);

	private static final DynamicLongProperty maxRetryMs =
			DynamicPropertyFactory.getInstance().getLongProperty("dynomitemanager.storage.isAlive.retry.max.ms", 30000L);

	private static final DynamicIntProperty jedisConnectTimeoutMs =
			DynamicPropertyFactory.getInstance().getIntProperty("dynomitemanager.storage.jedis.connect.timeout.ms", 30000);
